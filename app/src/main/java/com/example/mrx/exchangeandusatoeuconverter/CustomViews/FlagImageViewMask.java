package com.example.mrx.exchangeandusatoeuconverter.CustomViews;
...
public class FlagImageViewMask extends PorterImageView{

    private Drawable shape;
    private Matrix matrix;
    private Matrix drawMatrix;

    public FlagImageViewMask(Context context) {
        super(context);
        setup(context, null, 0);
    }

    public FlagImageViewMask(Context context, AttributeSet attrs) {
        super(context, attrs);
        setup(context, attrs, 0);
    }

    public FlagImageViewMask(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void setup(Context context, AttributeSet attrs, int defStyle) {
        if(attrs != null){
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.FlagImageViewMask, defStyle, 0);
            shape = typedArray.getDrawable(
                    R.styleable.FlagImageViewMask_siShape);
            typedArray.recycle();
        }
        matrix = new Matrix();
    }
